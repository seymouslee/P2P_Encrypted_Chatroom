...
class DataBase:
    ...
    def _create_key_table(self):
        """
        create new key table if one doesn't exist
        :return: None
        """
    ...
    def save_keys(self, name, pu, pk):
        """
        saves the keys generated by DH
        :param name: str
        :param pu: str
        :param pk: str
        :return: None
        """
        query = f"INSERT INTO {KEY_TABLE} VALUES (?, ?, ?, ?)"
        self.cursor.execute(query, (None, name, pu, pk))
        self.conn.commit()

    def get_all_keys(self, limit=100, name=None):
        """
        returns all keys
        :param limit: int
        :return: list[dict]
        """
        if not name:
            query = f"SELECT * FROM {KEY_TABLE}"
            self.cursor.execute(query)
        else:
            query = f"SELECT * FROM {KEY_TABLE} WHERE NAME = ?"
            self.cursor.execute(query, (name,))

        result = self.cursor.fetchall()
        print(result)

    def get_keys_by_name(self, name, limit=100):
        """
        Gets a list of messages by user name
        :param name: str
        :return: list
        """
        return self.get_all_keys(limit, name)
